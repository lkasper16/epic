...
struct moduleParamsStrct {
  moduleParamsStrct()
      : mod_BIwidth(0.), mod_BIheight(0.), mod_SWThick(0.), mod_TWThick(0.), mod_FWThick(0.), mod_BWThick(0.),
        mod_width(0.), mod_height(0.), mod_notchWidthAbsA(0.), mod_notchWidthAbsB(0.), mod_notchWidthAbsC(0.),
        mod_notchWidthScin(0.), mod_notchDepth(0.), mod_sepDepth(0.), mod_visStr(""), mod_regStr(""), mod_limStr("")
  {
  }
  moduleParamsStrct(double BIwidth, double BIheight, double SWThick, double TWThick, double FWThick, double BWThick,
                    double width, double height, double notchWidthAbsA, double notchWidthAbsB, double notchWidthAbsC,
                    double notchWidthScin, double notchDepth, double sepDepth, std::string visStr, std::string regStr,
                    std::string limStr)
  {
    mod_BIwidth        = BIwidth;
...
struct sliceParamsStrct {
  sliceParamsStrct()
      : layer_ID(0), slice_ID(0), slice_partID(0), slice_thick(0.), slice_offset(0.), slice_matStr(""),
        slice_visStr(""), slice_regStr(""), slice_limStr("")
  {
  }
  sliceParamsStrct(int l_ID, int sl_ID, int sl_partID, double sl_thick, double sl_off, std::string sl_matStr,
                   std::string sl_visStr, std::string sl_regStr, std::string sl_limStr)
  {
    layer_ID     = l_ID;
    slice_ID     = sl_ID;
...
//************************************************************************************************************
Volume createAbsorberPlateEightM(Detector& desc, std::string basename, double h_mod, double w_mod, double t_mod_tp,
                                 double t_mod_sp, double t_slice, double h_notch, double w_notchA, double w_notchB,
                                 double w_notchC, Material slice_mat, std::string region, std::string limit,
                                 std::string vis, bool renderComp)
{
...
//************************************************************************************************************
Volume createAbsorberPlateFourM(Detector& desc, std::string basename, double h_mod, double w_mod, double t_mod_tp,
                                double t_mod_sp, double t_slice, double h_notch, double w_notchA, double w_notchC,
                                Material slice_mat, std::string region, std::string limit, std::string vis,
                                bool renderComp)
{
  // 0                 1                         2                          3            4
  const std::vector<double> xCoord = {-(w_mod / 2 - t_mod_sp), -(w_mod / 2 - t_mod_sp - w_notchC),
                                      -(w_mod / 2 - t_mod_sp - w_notchC), -w_notchA / 2, -w_notchA / 2,

                                      // 5                  6                         7                            8 9
                                      w_notchA / 2, w_notchA / 2, w_mod / 2 - t_mod_sp - w_notchC,
                                      w_mod / 2 - t_mod_sp - w_notchC, w_mod / 2 - t_mod_sp,
                                      // 10                   11                          12 13           14
                                      w_mod / 2 - t_mod_sp, w_mod / 2 - t_mod_sp - w_notchC,
                                      w_mod / 2 - t_mod_sp - w_notchC, w_notchA / 2, w_notchA / 2,
                                      // 15          16          17                         18 19
                                      -w_notchA / 2, -w_notchA / 2, -(w_mod / 2 - t_mod_sp - w_notchC),
                                      -(w_mod / 2 - t_mod_sp - w_notchC), -(w_mod / 2 - t_mod_sp)};
  // 0                 1                         2                      3                           4
  const std::vector<double> yCoord = {
      h_mod / 2 - t_mod_tp, h_mod / 2 - t_mod_tp, h_mod / 2 - t_mod_tp - h_notch, h_mod / 2 - t_mod_tp - h_notch,
      h_mod / 2 - t_mod_tp,
      // 5                  6                         7                            8                    9
      h_mod / 2 - t_mod_tp, h_mod / 2 - t_mod_tp - h_notch, h_mod / 2 - t_mod_tp - h_notch, h_mod / 2 - t_mod_tp,
      h_mod / 2 - t_mod_tp,
      // 10                   11                      12                              13                          14
      -(h_mod / 2 - t_mod_tp), -(h_mod / 2 - t_mod_tp), -(h_mod / 2 - t_mod_tp - h_notch),
      -(h_mod / 2 - t_mod_tp - h_notch), -(h_mod / 2 - t_mod_tp),
      // 15                   16                          17                              18                        19
      -(h_mod / 2 - t_mod_tp), -(h_mod / 2 - t_mod_tp - h_notch), -(h_mod / 2 - t_mod_tp - h_notch),
      -(h_mod / 2 - t_mod_tp), -(h_mod / 2 - t_mod_tp)};
...
//************************************************************************************************************
Volume createFillerPlateEightM(Detector& desc, std::string basename, double h_mod, double w_mod, double t_mod_tp,
                               double t_mod_sp, double t_slice, double h_notch, double w_notch, Material slice_mat,
                               std::string region, std::string limit, std::string vis, bool renderComp)
{
  double w_plate = w_mod - 2 * t_mod_sp;
  double h_plate = h_mod - 2 * t_mod_tp;
  // 0                 1                         2                          3                       4
  const std::vector<double> xCoord = {-w_plate / 2, -(w_plate / 4 + w_notch / 2), -(w_plate / 4 + w_notch / 2),
                                      -(w_plate / 4 - w_notch / 2), -(w_plate / 4 - w_notch / 2),
                                      // 5                    6                         7                            8 9
                                      w_plate / 4 - w_notch / 2, w_plate / 4 - w_notch / 2, w_plate / 4 + w_notch / 2,
                                      w_plate / 4 + w_notch / 2, w_plate / 2,
                                      // 10                   11                    12                        13 14
                                      w_plate / 2, w_plate / 4 + w_notch / 2, w_plate / 4 + w_notch / 2,
                                      w_plate / 4 - w_notch / 2, w_plate / 4 - w_notch / 2,
                                      // 15                   16                        17                      18 19
                                      -(w_plate / 4 - w_notch / 2), -(w_plate / 4 - w_notch / 2),
                                      -(w_plate / 4 + w_notch / 2), -(w_plate / 4 + w_notch / 2), -w_plate / 2};
  // 0                     1                  2              3             4
  const std::vector<double> yCoord = {
...
//************************************************************************************************************
Volume createFillerPlateFourM(Detector& desc, std::string basename, double h_mod, double w_mod, double t_mod_tp,
                              double t_mod_sp, double t_slice, double h_notch, double w_notch, Material slice_mat,
                              std::string region, std::string limit, std::string vis, bool renderComp)
{
  // 0                 1                         2                          3            4
  const std::vector<double> xCoord = {-(w_mod / 2 - t_mod_sp), -w_notch / 2, -w_notch / 2, w_notch / 2, w_notch / 2,
                                      ...
                                      // 5                  6                         7                            8 9
                                      w_mod / 2 - t_mod_sp, w_mod / 2 - t_mod_sp, w_notch / 2, w_notch / 2,
                                      -w_notch / 2,
                                      // 10                   11
                                      -w_notch / 2, -(w_mod / 2 - t_mod_sp)};
  // 0                           1                         2                   3                4
  const std::vector<double> yCoord = {h_mod / 2 - t_mod_tp - h_notch, h_mod / 2 - t_mod_tp - h_notch,
                                      h_mod / 2 - t_mod_tp, h_mod / 2 - t_mod_tp, h_mod / 2 - t_mod_tp - h_notch,
                                      // 5                          6                            7 8 9
...
//************************************************************************************************************
Volume createScintillatorTower(Detector& desc, std::string basename, double w_tow, double h_tow, double t_slice,
                               double h_notch, double w_notch, Material slice_mat, std::string region,
                               std::string limit, std::string vis, SensitiveDetector sens, bool renderComp)
{
  // 0                1,              2,                3,          4
...
//************************************************************************************************************
Assembly createScintillatorPlateEightM(Detector& desc, std::string basename, int modID, int layerID, double h_mod,
                                       double w_mod, double t_mod_tp, double t_mod_sp, double t_slice, double h_notch,
                                       double w_notch, double t_sep, Material slice_mat, std::string region,
                                       std::string limit, std::string vis, SensitiveDetector sens, bool renderComp)
{
  ...
  const std::vector<double> xCoordTi = {
      -(w_mod / 2 - t_mod_sp), -(w_tow + 1.5 * t_sep), -(w_tow + 1.5 * t_sep), -(w_tow + 0.5 * t_sep),
      -(w_tow + 0.5 * t_sep),
      // 5          6          7            8           9
  ...
  const std::vector<double> yCoordTi = {
      t_sep / 2., t_sep / 2., (h_tow + h_notch + t_sep / 2), (h_tow + h_notch + t_sep / 2), t_sep / 2.,
      // 5            6                    7                         8            9
  ...
  Volume ti0grid_vol(basename + "_Ti02Epoxy_" + _toString(layerID, "_lyr_%d"), tiOgrid, slice_mat);
  // Setting slice attributes
  if (renderComp) {
  ...
  modScintAssembly.placeVolume(ti0grid_vol, Position(0, 0, 0));
  ...
  double posX[8] = {(w_tow * 1.5 + 1.5 * t_sep),  (w_tow * 0.5 + 0.5 * t_sep), -(w_tow * 0.5 + 0.5 * t_sep),
                    -(w_tow * 1.5 + 1.5 * t_sep), (w_tow * 1.5 + 1.5 * t_sep), (w_tow * 0.5 + 0.5 * t_sep),
                    -(w_tow * 0.5 + 0.5 * t_sep), -(w_tow * 1.5 + 1.5 * t_sep)};
  double posY[8] = {0.5 * (h_tow) + 0.5 * t_sep,    0.5 * (h_tow) + 0.5 * t_sep,    0.5 * (h_tow) + 0.5 * t_sep,
                    0.5 * (h_tow) + 0.5 * t_sep,    -(0.5 * (h_tow) + 0.5 * t_sep), -(0.5 * (h_tow) + 0.5 * t_sep),
                    -(0.5 * (h_tow) + 0.5 * t_sep), -(0.5 * (h_tow) + 0.5 * t_sep)};
  double posZ[8] = {0, 0, 0, 0, 0, 0, 0, 0};
  // loop over all towers within same module
  for (int i = 0; i < 8; i++) {
    if(i==0){
      std::cout << basename << _toString(i, "_tower_%d") << "\t"<< modID << "\t" << i << "\t"<< layerID <<
      std::endl;
    }
    Volume modScintTowerAss =
        createScintillatorTower(desc, basename + _toString(i, "_tower_%d"), w_tow, h_tow, t_slice, h_notch,
                                (w_notch - t_sep) / 2, slice_mat, region, limit, vis, sens, renderComp);
    pvm = modScintAssembly.placeVolume(
        modScintTowerAss, Transform3D(RotationZYX(rotZ[i], rotY[i], rotX[i]), Position(posX[i], posY[i], posZ[i])));
    pvm.addPhysVolID("module", modID);
    pvm.addPhysVolID("tower", i);
    pvm.addPhysVolID("layer", layerID);
  }
...
//************************************************************************************************************
Assembly createScintillatorPlateFourM(Detector& desc, std::string basename, int modID, int layerID, double h_mod,
                                      double w_mod, double t_mod_tp, double t_mod_sp, double t_slice, double h_notch,
                                      double w_notch, double t_sep, Material slice_mat, std::string region,
                                      std::string limit, std::string vis, SensitiveDetector sens, bool renderComp)
{
  ...
  const std::vector<double> xCoordTi = {-(w_mod / 2 - t_mod_sp), -(t_sep / 2.), -(t_sep / 2.), t_sep / 2., t_sep / 2.,
                                        // 5                6                7                8           9
                                        w_mod / 2 - t_mod_sp, w_mod / 2 - t_mod_sp, t_sep / 2., t_sep / 2.,
                                        -(t_sep / 2.),
                                        // 10                11
  ...
  double posX[4] = {(w_tow * 0.5 + 0.5 * t_sep), -(w_tow * 0.5 + 0.5 * t_sep), (w_tow * 0.5 + 0.5 * t_sep),
                    -(w_tow * 0.5 + 0.5 * t_sep)};
  double posY[4] = {0.5 * (h_tow) + 0.5 * t_sep, 0.5 * (h_tow) + 0.5 * t_sep, -(0.5 * (h_tow) + 0.5 * t_sep),
                    -(0.5 * (h_tow) + 0.5 * t_sep)};
...
//************************************************************************************************************
Volume createEightMModule(Detector& desc, moduleParamsStrct mod_params, std::vector<sliceParamsStrct> sl_params,
                          int modID, double length, SensitiveDetector sens, bool renderComp)
{
  ...
  Box modSidePlateR(mod_params.mod_SWThick / 2., mod_params.mod_height / 2.,
                    (length - mod_params.mod_FWThick - mod_params.mod_BWThick) / 2.);
  Box modTopPlate((mod_params.mod_width - 2 * mod_params.mod_SWThick) / 2., mod_params.mod_TWThick / 2.,
                  (length - mod_params.mod_FWThick - mod_params.mod_BWThick) / 2.);
  Box modBottomPlate((mod_params.mod_width - 2 * mod_params.mod_SWThick) / 2., mod_params.mod_TWThick / 2.,
                     (length - mod_params.mod_FWThick - mod_params.mod_BWThick) / 2.);
  ...
  for (int i = 0; i < (int)sl_params.size(); i++) {
    ...
    if (sl_params[i].slice_partID == 1) {
      ...
      //                                                     baseName+"_Abs"+_toString(sl_params[i].layer_ID,
      //                                                     "_lyr_%d"), mod_params.mod_height, mod_params.mod_width,
      //                                                     mod_params.mod_TWThick, mod_params.mod_SWThick,
      //                                                     sl_params[i].slice_thick, mod_params.mod_notchDepth,
      //                                                     mod_params.mod_notchWidthAbsA,
      //                                                     mod_params.mod_notchWidthAbsB,
      //                                                     mod_params.mod_notchWidthAbsC, slice_mat,
      //                                                     sl_params[i].slice_regStr, sl_params[i].slice_limStr,
      //                                                     sl_params[i].slice_visStr, renderComp);
      // // Placing slice within layer
      // vol_mod.placeVolume(modAbsAssembly, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., slice_z)));
      //*************************************************
      // air & tyvek
      //*************************************************
    } else if (sl_params[i].slice_partID == 2) {
      // Volume modFillAssembly =  createFillerPlateEightM( desc,
      //                                                     baseName+"_Fill"+_toString(sl_params[i].layer_ID,
      //                                                     "_lyr_%d")+_toString(sl_params[i].slice_ID, "slice_%d"),
      //                                                     mod_params.mod_height, mod_params.mod_width,
      //                                                     mod_params.mod_TWThick, mod_params.mod_SWThick,
      //                                                     sl_params[i].slice_thick, mod_params.mod_notchDepth,
      //                                                     mod_params.mod_notchWidthScin, slice_mat,
      //                                                     sl_params[i].slice_regStr, sl_params[i].slice_limStr,
      //                                                     sl_params[i].slice_visStr, renderComp);
      // // Placing slice within layer
      // vol_mod.placeVolume(modFillAssembly, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., slice_z)));
      //*************************************************
      // scintillator
      //*************************************************
    } else {
      Assembly modScintAssembly = createScintillatorPlateEightM(
          desc, baseName + "_SciAsy" + _toString(sl_params[i].layer_ID, "_lyr_%d"), modID, layer_num,
      ...
      // Placing slice within layer
      vol_mod.placeVolume(modScintAssembly, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., slice_z)));
    }
    slice_z += sl_params[i].slice_thick / 2.;
  }

  // placement 8M module casing
  vol_mod.placeVolume(vol_modFrontPlate, Position(0, 0, -(length - mod_params.mod_FWThick) / 2.));
  vol_mod.placeVolume(vol_modBackPlate, Position(0, 0, (length - mod_params.mod_BWThick) / 2.));
  vol_mod.placeVolume(vol_modSidePlateL, Position(-(mod_params.mod_width - mod_params.mod_SWThick) / 2., 0,
                                                        (mod_params.mod_FWThick - mod_params.mod_BWThick) / 2));
  vol_mod.placeVolume(vol_modSidePlateR, Position((mod_params.mod_width - mod_params.mod_SWThick) / 2., 0,
                                                        (mod_params.mod_FWThick - mod_params.mod_BWThick) / 2));
  vol_mod.placeVolume(vol_modTopPlate, Position(0, (mod_params.mod_height - mod_params.mod_TWThick) / 2.,
                                                      (mod_params.mod_FWThick - mod_params.mod_BWThick) / 2));
  vol_mod.placeVolume(vol_modBottomPlate, Position(0, -(mod_params.mod_height - mod_params.mod_TWThick) / 2.,
                                                         (mod_params.mod_FWThick - mod_params.mod_BWThick) / 2));
...
//************************** create 8M module assembly  ******************************************************
//************************************************************************************************************
Volume createFourMModule(Detector& desc, moduleParamsStrct mod_params, std::vector<sliceParamsStrct> sl_params,
                         int modID, double length, SensitiveDetector sens, bool renderComp)
{
  ...
  for (int i = 0; i < (int)sl_params.size(); i++) {
    ...
    Material slice_mat = desc.material(sl_params[i].slice_matStr);
    if (sl_params[i].slice_partID == 1) {
      Volume modAbsAssembly = createAbsorberPlateFourM(
          desc, baseName + "_Abs" + _toString(sl_params[i].layer_ID, "_lyr_%d"), mod_params.mod_height,
          mod_params.mod_width, mod_params.mod_TWThick, mod_params.mod_SWThick, sl_params[i].slice_thick,
          mod_params.mod_notchDepth, mod_params.mod_notchWidthAbsA, mod_params.mod_notchWidthAbsC, slice_mat,
      ...
      Volume modFillAssembly = createFillerPlateFourM(
          desc,
          baseName + "_Fill" + _toString(sl_params[i].layer_ID, "_lyr_%d") +
              _toString(sl_params[i].slice_ID, "slice_%d"),
          mod_params.mod_height, mod_params.mod_width, mod_params.mod_TWThick, mod_params.mod_SWThick,
          sl_params[i].slice_thick, mod_params.mod_notchDepth, mod_params.mod_notchWidthScin, slice_mat,
          sl_params[i].slice_regStr, sl_params[i].slice_limStr, sl_params[i].slice_visStr, renderComp);
      // Placing slice within layer
      vol_mod.placeVolume(modFillAssembly, Transform3D(RotationZYX(0, 0, 0), Position(0., 0., slice_z)));
      //*************************************************
      ...
      Assembly modScintAssembly = createScintillatorPlateFourM(
          desc, baseName + "_ScintAssembly" + _toString(sl_params[i].layer_ID, "_lyr_%d"), modID, layer_num,
          mod_params.mod_height, mod_params.mod_width, mod_params.mod_TWThick, mod_params.mod_SWThick,
          sl_params[i].slice_thick, mod_params.mod_notchDepth, mod_params.mod_notchWidthScin, mod_params.mod_sepDepth,
          slice_mat, sl_params[i].slice_regStr, sl_params[i].slice_limStr, sl_params[i].slice_visStr, sens, renderComp);
      // Placing slice within layer
...
static Ref_t createDetector(Detector& desc, xml_h handle, SensitiveDetector sens)
{
  ...
  moduleParamsStrct eightM_params(getAttrOrDefault(eightMmod_dim, _Unicode(widthBackInner), 0.),
                                  ...
                                  getAttrOrDefault(eightMmod_dim, _Unicode(sepDepth), 0.), eightM_xml.visStr(),
  ...
  moduleParamsStrct fourM_params(
      ...
      getAttrOrDefault(fourMmod_dim, _Unicode(width), 0.), getAttrOrDefault(fourMmod_dim, _Unicode(height), 0.),
      ...
      getAttrOrDefault(fourMmod_dim, _Unicode(notchDepth), 0.), getAttrOrDefault(fourMmod_dim, _Unicode(sepDepth), 0.),
  ...
  for (xml_coll_t c(x_det, _U(layer)); c; ++c) {
    ...
    for (int i = 0; i < repeat; i++) {
      ...
      for (xml_coll_t l(x_layer, _U(slice)); l; ++l) {
        ...
        sliceParamsStrct slice_param(layer_num, slice_num, getAttrOrDefault(l, _Unicode(type), 0.), x_slice.thickness(),
                                     getAttrOrDefault(l, _Unicode(offset), 0.), x_slice.materialStr(), x_slice.visStr(),
  ...
  if (xpos8M.size() != ypos8M.size() || xpos8M.size() != zpos8M.size()) {
    ...
    for (int e = 0; e < (int)xpos8M.size(); e++) {
      if (e % 20 == 0)
        std::cout << "LFHCAL placing 8M module: " << e << "/" << (int)xpos8M.size() << "\t" << xpos8M[e] << "\t"
                  << ypos8M[e] << "\t" << zpos8M[e] << std::endl;
      Volume eightMassembly =
          createEightMModule(desc, eightM_params, slice_Params, moduleID, length, sens, renderComponents);

      // Placing modules in world volume
      //       auto tr8M = Transform3D(Position(pos.x()-xpos8M[e]*dd4hep::cm-0.5*eightM_params.mod_width,
      //       pos.y()-ypos8M[e]*dd4hep::cm-0.5*eightM_params.mod_height, pos.z() +zpos8M[e]*dd4hep::cm + length / 2.));
      auto tr8M =
          Transform3D(Position(pos.x() - xpos8M[e] * dd4hep::cm - 0.5 * eightM_params.mod_width,
  ...
  if (xpos4M.size() != ypos4M.size() || xpos4M.size() != zpos4M.size()) {
    ...
    for (int f = 0; f < (int)xpos4M.size(); f++) {
      if (f % 20 == 0)
        std::cout << "LFHCAL placing 4M module: " << f << "/" << (int)xpos4M.size() << "\t" << xpos4M[f] << "\t"
                  << ypos4M[f] << "\t" << zpos4M[f] << std::endl;
      Volume fourMassembly =
          createFourMModule(desc, fourM_params, slice_Params, moduleID, length, sens, renderComponents);
      // Placing modules in world volume
      //       auto tr4M = Transform3D(Position(pos.x()-xpos4M[f]*dd4hep::cm-0.5*fourM_params.mod_width,
      //       pos.y()-ypos4M[f]*dd4hep::cm-0.5*fourM_params.mod_height, pos.z() +zpos4M[f]*dd4hep::cm + length / 2.));
      auto tr4M =
          Transform3D(Position(pos.x() - xpos4M[f] * dd4hep::cm - 0.5 * fourM_params.mod_width,
                               pos.y() - ypos4M[f] * dd4hep::cm, pos.z() + zpos4M[f] * dd4hep::cm + length / 2.));
